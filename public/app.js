...
function noteAt(f) {
  ...
  return [
    `${NOTE[(m % 12 + 12) % 12]}${Math.floor(m / 12) - 1}`,
    440 * 2 ** ((m - 69) / 12)
  ];
}
...
function sx(x) {
  return (x - view.x0) / (view.x1 - view.x0) * canvas.clientWidth;
}

function sy(y) {
  return canvas.clientHeight - (y - view.y0) / (view.y1 - view.y0) * canvas.clientHeight;
}

function tx(px) {
  return view.x0 + px / canvas.clientWidth * (view.x1 - view.x0);
}

function ty(py) {
  return view.y0 + (canvas.clientHeight - py) / canvas.clientHeight * (view.y1 - view.y0);
}
...
function draw() {
  ...
  if ($('sampled').checked) {
    for (let i = 0; i < 3; i++) {
      line(data.sampledTrack.map(p => [p[0], p[1][i]]), 'rgba(168,85,247,.55)', 1.2);
    }
  }
  ...
  if ($('beatNotes').checked) {
    for (const b of notes) {
      ...
      // Draw the delete dot AFTER the box, with its center above the box.
      // It is intentionally outside the rectangle so the corner is never covered.
      if ($('dismiss').checked) {
        ctx.save();
  ...
  if (playPos > 0) {
    ...
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(sx(playPos), 0);
    ctx.lineTo(sx(playPos), h);
    ctx.stroke();
  }
...
async function loadFile(f) {
  ...
  const r = await fetch('/api/analyze?win=2048&hop=512', {
    method: 'POST',
...
$('file').onchange = e => {
  if (e.target.files[0]) {
    loadFile(e.target.files[0]).catch(err => {
...
for (const id of ['noteLines', 'sampled', 'beatLines', 'beatNotes', 'dismiss', 'bpm', 'meter']) {
  $(id).addEventListener('input', () => {
    if (data && (id === 'bpm' || id === 'meter')) makeNotes();
    draw();
  });
}

$('reset').onclick = () => {
  if (data) view = { x0: 0, x1: data.duration, y0: 0, y1: data.fMax };
  draw();
};
...
canvas.addEventListener('wheel', e => {
  if (!data) return;
  e.preventDefault();
  const cx = tx(e.offsetX);
  const cy = ty(e.offsetY);
  const k = e.deltaY > 0 ? 1.12 : .88;
...
canvas.onmousemove = e => {
  ...
  view.x0 = Math.max(0, drag.x0 - dx);
  view.x1 = Math.min(data.duration, drag.x1 - dx);
  view.y0 = Math.max(0, drag.y0 + dy);
  view.y1 = Math.min(data.fMax, drag.y1 + dy);
  draw();
};
...
canvas.onclick = e => {
  ...
  // Delete-dot hit testing is done in SCREEN/CANVAS PIXELS, not frequency space.
  // The old code compared the dot's pixel Y with sy(28), which is why clicks missed.
  if ($('dismiss').checked) {
    for (const b of notes) {
      if (b.t < view.x0 - beatDur || b.t > view.x1) continue;
      const bx = sx(b.t);
...
function synthTrack(points, duration, sr = 44100) {
  ...
  const p = mode === 'max'
    ? data.track
    : mode === 's1'
      ? data.sampledTrack.map(x => [x[0], x[1][0]])
      : mode === 's2'
        ? data.sampledTrack.map(x => [x[0], x[1][1]])
        : data.sampledTrack.map(x => [x[0], x[1][2]]);
...
function restart(pos) {
  ...
  if (mode === 'original') {
    getOriginal().then(b => {
      ...
      playBuffer(ch, b.sampleRate, pos);
    });
  } else if (mode === 'notes') {
    playBuffer(synthNotes(), 44100, pos);
  } else {
    playBuffer(synthTrack(data.track, data.duration), 44100, pos);
  }
}
...
function synthNotes() {
  ...
  for (const b of notes) {
    ...
    for (let i = 0; i < len && start + i < out.length; i++) {
      ...
      out[start + i] += .3 * env * Math.sin(2 * Math.PI * f * i / sr);
    }
  }
  return out;
}
